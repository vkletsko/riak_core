...
random_other_node(OrdSet) ->
    ...
    case Size of
        0 -> undefined;
        _ ->
            lists:nth(riak_core_util:rand_uniform(Size),
...
init_peers(Members) ->
    case length(Members) of
        ...
        N when N < 5 ->
            ...
            InitLazys  = [lists:nth(
                riak_core_util:rand_uniform(N - 2), Members -- [node() | InitEagers])];
        N when N < 10 ->
            ...
            InitEagers = orddict:fetch(node(), Tree),
            InitLazys  = [lists:nth(
                riak_core_util:rand_uniform(N - 3), Members -- [node() | InitEagers])];
        N ->
            ...
            InitEagers = orddict:fetch(node(), Tree),
            InitLazys  = [lists:nth(
                riak_core_util:rand_uniform(N - (NEagers + 1)), Members -- [node() | InitEagers])]
    end,
